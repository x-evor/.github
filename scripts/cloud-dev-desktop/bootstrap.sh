...
args=(ansible-playbook -i "${inventory_file}" -D "${repo_root}/ansible/playbooks/bootstrap_cloud_dev_desktop.yml" -e "@${runtime_vars}")
if [[ "${mode}" == "dry-run" ]]; then
  args=(ansible-playbook -i "${inventory_file}" -D -C "${repo_root}/ansible/playbooks/bootstrap_cloud_dev_desktop.yml" -e "@${runtime_vars}")
fi
